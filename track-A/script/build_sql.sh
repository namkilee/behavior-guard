...
perl -0777 -pe 's{/\*__SESSIONS_QUERY__\*/}{$ENV{SESSIONS_QUERY}}g' \
  -- \
  <(SESSIONS_QUERY="$SESSIONS_QUERY" cat "$RAW_TEMPLATE_FILE") \
  > "$OUT_RAW"

perl -0777 -pe 's{/\*__SESSIONS_QUERY__\*/}{$ENV{SESSIONS_QUERY}}g' \
  -- \
  <(SESSIONS_QUERY="$SESSIONS_QUERY" cat "$FEAT_TEMPLATE_FILE") \
  > "$OUT_FEAT"

# quick sanity: placeholder leftovers
